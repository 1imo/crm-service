...
export default function ViewFloorplans({ params }: { params: { customerId: string } }) {
  ...
  useEffect(() => {
    ...
    const url = `${process.env.NEXT_PUBLIC_ROOMING_SERVICE_URL}/floorplan/${session.user.companyId}/${params.customerId}`;
    setIframeUrl(url);
    setLoading(false);
  ...
  const handlePrint = async () => {
    ...
    const pdfUrl = `${process.env.NEXT_PUBLIC_ROOMING_SERVICE_URL}/floor/pdf/${session.user.companyId}/${params.customerId}`;
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const filename = contentDisposition?.split('filename=')[1]?.replace(/"/g, '') || `floorplan-${params.customerId}.pdf`;
  ...
  const handleSendToClient = async () => {
    try {
      const url = `${process.env.NEXT_PUBLIC_ROOMING_SERVICE_URL}/floorplan/send-to-client/${session?.user?.companyId}/${params.customerId}`;
